...
namespace PussyCats.Web.ServiceProxies
{
    public class SkillTestServiceProxy : ISkillTestService
    {
        ...
        {
            return await httpClient.GetFromJsonAsync<List<SkillTest>>($"api/skill-tests?userId={userId}", cancellationToken) ?? new List<SkillTest>();
        }

        public async Task<SkillTest?> GetSkillTestByIdAsync(int skillTestId, CancellationToken cancellationToken = default)
        {
            return await httpClient.GetFromJsonAsync<SkillTest>($"api/skill-tests/{skillTestId}", cancellationToken);
        }
        ...
        {
            var response = await httpClient.PostAsJsonAsync($"api/skill-tests/{skillTestId}/retake", new { Score = newScore }, cancellationToken);
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadFromJsonAsync<Badge>(cancellationToken: cancellationToken) ?? throw new InvalidOperationException("Response content was null");
        }
    }
}
