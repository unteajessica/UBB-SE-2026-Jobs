...
public class CompanyRepositoryProxy : ICompanyRepository
{
    ...
    {
        return await RepositoryProxyJson.GetOrNullAsync<Company>(http, $"api/companies/{companyId}", ct).ConfigureAwait(false);
    }

    public async Task<IReadOnlyList<Company>> GetAllAsync(CancellationToken ct = default)
    {
        return await RepositoryProxyJson.GetListAsync<Company>(http, "api/companies", ct).ConfigureAwait(false);
    }

    public async Task<Company> AddAsync(Company company, CancellationToken ct = default)
    {
        using var response = await http.PostAsJsonAsync("api/companies", company, RepositoryProxyJson.Options, ct).ConfigureAwait(false);
        return await RepositoryProxyJson.ReadRequiredAsync<Company>(response, ct).ConfigureAwait(false);
    }

    public async Task UpdateAsync(Company company, CancellationToken ct = default)
    {
        using var response = await http.PutAsJsonAsync($"api/companies/{company.CompanyId}", company, RepositoryProxyJson.Options, ct).ConfigureAwait(false);
    ...
    {
        using var response = await http.DeleteAsync($"api/companies/{companyId}", ct).ConfigureAwait(false);
        await RepositoryProxyJson.SendAndIgnoreNotFoundAsync(response).ConfigureAwait(false);
    }
}
