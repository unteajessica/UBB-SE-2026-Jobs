...
public class FakeMatchRepository : IMatchRepository
{
    ...
    {
        IReadOnlyList<Match> filtered = matchesById.Values.Where(match => match.UserId == userId).ToList();
        return Task.FromResult(filtered);
    }

    public Task<Match?> GetByUserIdAndJobIdAsync(int userId, int jobId, CancellationToken cancellationToken = default)
    {
        var match = matchesById.Values.FirstOrDefault(match => match.UserId == userId && match.JobId == jobId);
        return Task.FromResult(match);
    }
